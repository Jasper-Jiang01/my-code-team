...
降低 HTTP 客户端与 TLS 建连成本。不同节点可通过 ``max_retries`` / ``timeout``
参数覆盖默认策略。

模型路由规则（按模型名称前缀）：
- ``claude*``    → ChatAnthropic（需 anthropic_api_key）
- ``longcat*``   → ChatOpenAI 指向 LongCat 的 OpenAI 兼容接口（需 longcat_api_key）
- 其他           → ChatOpenAI（需 openai_api_key）
"""
...
def create_model(
    ...
    *,
    ...
    logger.debug("Initializing chat model %r (timeout=%ss, max_retries=%s)", name, timeout, max_retries)
    return _create_chat_model_cached(name, timeout, max_retries)


# 向后兼容别名：统一入口为 create_model；init_chat_model 对齐
# langchain 生态命名，create_chat_model 兼容旧代码。
init_chat_model = create_model
create_chat_model = create_model
